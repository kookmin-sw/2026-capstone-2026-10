...
def wy(y: float, min_y: float, max_y: float | None = None) -> float:
    if max_y is not None:
        return PADDING + (max_y - y) * SCALE
...
def outer_wall_extension_rect(
    ...
    x1, y1 = wx(edge["x1"], min_x), wy(edge["y1"], min_y, max_y)
    x2, y2 = wx(edge["x2"], min_x), wy(edge["y2"], min_y, max_y)
...
def outer_corner_extension_rects(
    ...
    for edge in outer_edges:
        points = (
            (wx(edge["x1"], min_x), wy(edge["y1"], min_y, max_y)),
            (wx(edge["x2"], min_x), wy(edge["y2"], min_y, max_y)),
        )
        for x, y in points:
...
WIDE_OPENING_COLOR = "#555555"
WIDE_OPENING_JAMB = 10  # px — perpendicular tick length at each jamb end


def svg_wide_opening_jambs(x1: float, y1: float, x2: float, y2: float) -> list[str]:
    """Two short perpendicular jamb lines at each end of a wide opening.

    Visually distinct from the window symbol (which uses two parallel lines
    along the opening length in blue).  These are dark-grey vertical/horizontal
    ticks placed at the opening edges, like column reveals in an archway.
    """
    j = WIDE_OPENING_JAMB / 2
    stroke = WIDE_OPENING_COLOR
    sw = 2.0
    is_horiz = abs(y1 - y2) < 0.5
    if is_horiz:
        y = (y1 + y2) / 2
        xl, xr = min(x1, x2), max(x1, x2)
        # vertical ticks at left and right ends of the opening
        left  = f'<line x1="{xl:.1f}" y1="{y-j:.1f}" x2="{xl:.1f}" y2="{y+j:.1f}" stroke="{stroke}" stroke-width="{sw}" stroke-linecap="square"/>'
        right = f'<line x1="{xr:.1f}" y1="{y-j:.1f}" x2="{xr:.1f}" y2="{y+j:.1f}" stroke="{stroke}" stroke-width="{sw}" stroke-linecap="square"/>'
    else:
        x = (x1 + x2) / 2
        yt, yb = min(y1, y2), max(y1, y2)
        # horizontal ticks at top and bottom ends of the opening
        left  = f'<line x1="{x-j:.1f}" y1="{yt:.1f}" x2="{x+j:.1f}" y2="{yt:.1f}" stroke="{stroke}" stroke-width="{sw}" stroke-linecap="square"/>'
        right = f'<line x1="{x-j:.1f}" y1="{yb:.1f}" x2="{x+j:.1f}" y2="{yb:.1f}" stroke="{stroke}" stroke-width="{sw}" stroke-linecap="square"/>'
    return [left, right]


_DOOR_ARC_MAX_R = 32   # px — fixed arc radius so all doors look the same size
_PRIVATE_TYPES = {"bedroom", "master_bedroom", "child_bedroom", "bathroom", "workspace"}


def _space_centroid(spaces: list[dict], space_id: str) -> tuple[float, float] | None:
    for sp in spaces:
...
def _door_swing(
    ...
    """Return (hinge_side, swing_dir) for an interior door opening.

    hinge_side: 'left'|'right' for horizontal walls,
                'top'|'bottom' for vertical walls.
    swing_dir:  'down'|'up' for horizontal walls,
                'right'|'left' for vertical walls.

    Strategy: swing the door INTO the more private room, hinged at the end
    of the opening that is closer to the public/corridor space's centroid.
    """
    is_horiz = abs(op["y1"] - op["y2"]) < 0.1

    ca = _space_centroid(spaces, op.get("space_a", ""))
    cb = _space_centroid(spaces, op.get("space_b", ""))

    type_a = op.get("space_type_a", "")
    type_b = op.get("space_type_b", "")
    a_private = type_a in _PRIVATE_TYPES
    b_private = type_b in _PRIVATE_TYPES

    # Which centroid to swing TOWARD (private room side)
    # If b is more private (or neither), swing toward b.  If only a is private, toward a.
    toward = cb if (b_private or not a_private) else ca

    ox1, oy1, ox2, oy2 = op["x1"], op["y1"], op["x2"], op["y2"]

    if is_horiz:
        wall_y = (oy1 + oy2) / 2
        xl_plan = min(ox1, ox2)
        # swing direction: up if private room is NORTH of wall (plan y > wall_y)
        if toward is not None:
            swing_dir = "up" if toward[1] > wall_y else "down"
        else:
            swing_dir = "down"
        # hinge at left end (xl_plan side)
        if toward is not None:
            hinge = "left" if toward[0] >= xl_plan else "right"
        else:
            hinge = "left"
    else:
        wall_x = (ox1 + ox2) / 2
        yt_plan = min(oy1, oy2)
        if toward is not None:
            swing_dir = "right" if toward[0] > wall_x else "left"
        else:
            swing_dir = "right"
        if toward is not None:
            hinge = "top" if toward[1] >= yt_plan else "bottom"
        else:
            hinge = "top"

    return hinge, swing_dir
...
def svg_door_arc(
    ...
    """Quarter-circle door arc.

    The arc endpoint is always exactly r pixels from the hinge along the wall,
    so the SVG radius is never stretched.  All eight hinge/swing combinations
    are handled with a lookup table for the sweep-flag.
    """
    is_horiz = abs(y1 - y2) < 0.5
    stroke = "#888888"
    sw = 1.2
    ...
    hinge, swing_dir = (
        _door_swing(op, spaces or [])
        if op is not None
        else (("left" if is_horiz else "top"), ("down" if is_horiz else "right"))
    )

    # sweep-flag lookup: 0 = CCW, 1 = CW (in SVG screen coords, y↓)
    _sf: dict[tuple[str, str], int] = {
        ("left",   "down"):  0,   # 90°→0°   CCW
        ("left",   "up"):    1,   # 270°→0°  CW
        ("right",  "down"):  1,   # 90°→180° CW
        ("right",  "up"):    0,   # 270°→180° CCW
        ("top",    "right"): 1,   # 0°→90°   CW
        ("top",    "left"):  0,   # 180°→90° CCW
        ("bottom", "right"): 0,   # 0°→270°  CCW
        ("bottom", "left"):  1,   # 180°→270° CW
    }
    sf = _sf.get((hinge, swing_dir), 0)

    if is_horiz:
        y = (y1 + y2) / 2
        xl, xr = min(x1, x2), max(x1, x2)
        hx = xl if hinge == "left" else xr
        wall_sign = 1 if hinge == "left" else -1   # direction along wall from hinge
        swing_sign = 1 if swing_dir == "down" else -1
        # panel tip and arc endpoint are both exactly r px from hinge
        panel_y = y + swing_sign * r
        arc_end_x = hx + wall_sign * r             # ← KEY FIX: r px along wall, not full jamb
        path = (
            f"M {hx:.1f},{y:.1f} "
            f"L {hx:.1f},{panel_y:.1f} "
            f"A {r},{r} 0 0 {sf} {arc_end_x:.1f},{y:.1f}"
        )
    else:
        x = (x1 + x2) / 2
        yt, yb = min(y1, y2), max(y1, y2)
        hy = yt if hinge == "top" else yb
        wall_sign = 1 if hinge == "top" else -1    # direction along wall from hinge
        swing_sign = 1 if swing_dir == "right" else -1
        panel_x = x + swing_sign * r
        arc_end_y = hy + wall_sign * r             # ← KEY FIX: r px along wall
        path = (
            f"M {x:.1f},{hy:.1f} "
            f"L {panel_x:.1f},{hy:.1f} "
            f"A {r},{r} 0 0 {sf} {x:.1f},{arc_end_y:.1f}"
        )

    return [f'<path d="{path}" fill="none" stroke="{stroke}" stroke-width="{sw:.1f}"/>']
...
def build_svg(plan_geometry: dict[str, Any]) -> str:
    ...
    # ① 공간 채우기 (방 유형별 색상)
    for space in plan_geometry.get("spaces", []):
        pts = [(wx(x, min_x), wy(y, min_y, max_y)) for x, y in space["polygon"]]
    ...
    for e in outer_edges:
        x1, y1 = wx(e["x1"], min_x), wy(e["y1"], min_y, max_y)
        x2, y2 = wx(e["x2"], min_x), wy(e["y2"], min_y, max_y)
        els.append(svg_poly(wall_rect(x1, y1, x2, y2, INNER_WALL_T), WALL_COLOR))

    # ③ 외벽은 바깥 방향으로만 추가 두께를 더한다.
    for e in outer_edges:
        els.append(svg_poly(outer_wall_extension_rect(e, min_x, min_y, max_y), WALL_COLOR))

    for rect in outer_corner_extension_rects(outer_edges, min_x, min_y, max_y):
        els.append(svg_poly(rect, WALL_COLOR))

    # ④ 내부 벽
    inner_walls = plan_geometry.get("inner_walls", [])
    for w in plan_geometry.get("inner_walls", []):
        x1, y1 = wx(w["x1"], min_x), wy(w["y1"], min_y, max_y)
        x2, y2 = wx(w["x2"], min_x), wy(w["y2"], min_y, max_y)
        els.append(svg_poly(wall_rect(x1, y1, x2, y2, INNER_WALL_T), WALL_COLOR))

    # ⑤ 개구부 / 창문 — 벽 지우기 후 창문 선
    _spaces = plan_geometry.get("spaces", [])
    for op in plan_geometry.get("openings", []):
        x1, y1 = wx(op["x1"], min_x), wy(op["y1"], min_y, max_y)
        x2, y2 = wx(op["x2"], min_x), wy(op["y2"], min_y, max_y)
        ...
        # interior door/opening/wide_opening: gap only, no symbol

    # ⑥ 라벨 (room name + area)
    for label in plan_geometry.get("labels", []):
        lx = wx(label["x"], min_x)
        ly = wy(label["y"], min_y, max_y)
...
def build_png(plan_geometry: dict[str, Any]) -> Image.Image:
    ...
    # ③ 외벽은 바깥 방향으로만 추가 두께를 더한다.
    for e in outer_edges:
        _draw_wall_rect(draw, outer_wall_extension_rect(e, min_x, min_y, max_y), WALL_COLOR)
