...
def build_labels(
    ...
    for space in non_connector_spaces:
        ...
        # Area in m짼: grid unit = 0.5 m ??1 grid unit짼 = 0.25 m짼
...
def get_window_target_length(
    edge: dict[str, Any],
    edge_type: str,
    *,
    fallback: bool = False,
) -> tuple[float, float]:
    edge_len = segment_length(edge)

    if edge.get("space_type") == "living_room":
        return max(1.6, edge_len * 0.95), 0.12

    if fallback:
        ratio = 0.6 if edge_type == "window_required" else 0.45
        return min(WINDOW_DEFAULT_WIDTH, max(1.0, edge_len * ratio)), 0.3

    ratio = 0.8 if edge_type == "window_required" else 0.6
    return min(WINDOW_DEFAULT_WIDTH, max(1.2, edge_len * ratio)), 0.3
...
def build_outer_openings(outer_edges: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for edge in outer_edges:
        ...
        if edge_type == "entry":
            ...
            # window_required: 梨꾧킅 ?꾩닔 ??踰?湲몄씠?????볦? 鍮꾩쑉濡?李쎈Ц ?앹꽦
            target, min_margin = get_window_target_length(edge, edge_type)
            segment = centered_segment_on_edge(edge, target, min_margin=min_margin)
            if segment is None:
                continue
            x1, y1, x2, y2 = segment
            openings.append(
        ...
        # exterior: ?몃꼍?댁?留?李쎈Ц ?놁쓬 (?뺤떎 痢〓㈃, 二쇰갑 ?쒕퉬??踰???
        # solid, exterior 紐⑤몢 opening ?놁씠 洹몃?濡??몃꼍?쇰줈 ?뚮뜑留곷맖
...
def should_make_inner_opening(wall: dict[str, Any]) -> bool:
    ...
    return both_edges_connectable(edge_type_a, edge_type_b)
...
def build_inner_openings(
    ...
    for wall in inner_walls:
        ...
        # "open" access type means a boundary with no wall — skip entirely
        if access_type == "open":
            continue

        # Blocked pairs never get openings, even if layout data explicitly requests one
        if is_blocked_internal_opening_pair(wall["space_type_a"], wall["space_type_b"]):
            continue

        if use_access_edges and access_type is not None:
            # Explicit access type from layout — use it directly
            ...
            # No explicit access type: fall back to rule-based logic
...
def build_fallback_windows(
    ...
    for space_id, space_edges in edges_by_space.items():
        ...
        preferred_edge_type = "window_required" if nl_level == "required" else "window_preferred"
        target, min_margin = get_window_target_length(best_edge, preferred_edge_type, fallback=True)
        segment = centered_segment_on_edge(best_edge, target, min_margin=min_margin)
        if segment is None:
            continue
        x1, y1, x2, y2 = segment
        fallback.append(
